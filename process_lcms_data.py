...
def build_sheet1(wb, raw_data, ms_cols, S, cfg):
    ...
    ws.cell(row=2, column=stat_col, value=None)
    ws.cell(row=2, column=sd_col, value=None)
    ws.cell(row=2, column=se_col, value=None)
    ...
    for comp in ALL_COMPS:
        ...
        c_sd.value = f'=ROUND(STDEV.S({rr}),0)'; c_sd.number_format = '0'
        sty(c_sd, S['data'])
    ...
    for ss in SS_COMPS:
        ...
        for i, (_, cl, _) in enumerate(ms_cols):
            ...
            c = ws.cell(row=row, column=ms_start+i)
            if v is not None:
                c.value = round6(v / spike); c.number_format = '0.000000'
            sty(c, S['data'])
        ws.cell(row=row, column=mid1, value=None)
        ws.cell(row=row, column=rec_lbl, value=None)
        for i, (_, cl, _) in enumerate(ms_cols):
            ...
            c = ws.cell(row=row, column=rec_start+i)
            if v is not None:
                c.value = round_int((v / spike) * 100)  # ×100% 四舍五入取整
                c.number_format = '0'
            sty(c, S['rec'])
...
def build_sheet4(wb, raw_data, sample_cols, blank_info, s3_first, S, cfg):
    ...
    for i in range(n_s):
        ws.cell(row=2, column=sample_start+i, value=1)
    ...
    for comp in TARGET_COMPS:
        ...
        for col, pct in [(10,0.05),(11,0.25),(12,0.75),(13,0.95)]:
            c = ws.cell(row=row, column=col)
            c.value = f'=IF($D{row}>50%,PERCENTILE.INC({sr},{pct}),"NC")'
            c.number_format = '0.000000'
            sty(c, S['stat'])
...
def build_info_sheet(wb, S, cfg):
    ...
    rows = [
        ...
        ['Sheet1: avg/SD/SE','AVERAGE/STDEV.S/SD/SQRT(COUNT) 公式','Sheet1 回收率%列','基于回收率百分比值'],
