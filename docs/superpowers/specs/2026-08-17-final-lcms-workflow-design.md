...
8. For non-zero blanks, MDL is `mean(blank) + t(0.99, n_blank-1) × SD(blank)`; the one-sided 99% t value uses the actual valid blank count. The former low-level-spike input and MAX branch are removed. MQL is `mean(blank) + 10 × SD(blank)`.
...
12. The main interface includes a per-compound calculation-evidence area showing parsed blank values, counts, selected path, dynamic t inputs, substituted formula, result, and explicit reasons when calculation is unavailable.
